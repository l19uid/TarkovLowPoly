...
public class Weapon : MonoBehaviour
{
    ...
    [Header("Magazines")] private int bulletCount = 0;
    [SerializeField] public Magazine currentMagazine;
    [SerializeField] private int maxMagazines = 5;

    [Header("Stats")] [SerializeField] private float damage = 10;
    [SerializeField] private float fireRate = 10;
    [SerializeField] private float reloadTime = 2;

    [Header("Muzzle Flash")] public GameObject muzzleFlashEffect;

    [Header("Recoil")] [SerializeField] private Vector3 defaultPosition;
    [SerializeField] private Vector3 scopePosition;
    [SerializeField] private bool isScoped = false;
    [SerializeField] private float horzizontalRecoil = 10;
    [SerializeField] private float verticalRecoil = 10;
    [SerializeField] private float ergonomics = 10;
    ...
    [SerializeField] private float bulletSpeed = 100; //MpS
    [SerializeField] float bulletWeight = .2f;
    ...
    [SerializeField] float muzzleVelocity = 800f; // meters per second
    [SerializeField] float range = 1000f; // meters
    [SerializeField] float gravity = 9.81f;

    [SerializeField] CameraShake cameraShake;
    [SerializeField] GameObject bloodPFX;
    ...
    private void ManageFOW()
    {
        ...
        else
        {
            ...
            weaponCamera.fieldOfView = Mathf.Lerp(weaponCamera.fieldOfView, 90, Time.deltaTime * 25);
        }
    }
    ...
    private IEnumerator Shoot()
    {
        ...
        if (Random.Range(0, 6) < 4)
            Destroy(Instantiate(muzzleFlashEffect, muzzlePoint.position, Quaternion.identity), 1f);
    ...
    private IEnumerator BulletTravel(float distance, Vector3 dropDirection)
    {
        ...
        if (Physics.Raycast(muzzlePoint.position, dropDirection, out hit, range))
        {
            if (hit.transform.GetComponent<Health>() != null)
            {
                ...
                Destroy(Instantiate(bloodPFX, hit.point, Quaternion.identity), 1f);
            }
            ...
            while (time > 0)
            {
                bullet.transform.position += Vector3.Lerp(bullet.transform.position + dropDirection, hit.point,
                    Time.deltaTime * distance / bulletSpeed);
                time -= Time.deltaTime;
            }
        }
    }
    ...
    private void ManageRecoil()
    {
        ...
        recoilPosition = Vector3.Lerp(recoilPosition, resetPosition, Time.deltaTime * 3 );
        recoilRotation = Vector3.Lerp(recoilRotation, originalRotation.eulerAngles, Time.deltaTime * 3);

        keyboardInputRotation =
            Vector3.Lerp(keyboardInputRotation, originalRotation.eulerAngles, Time.deltaTime * ergonomics);

        mouseInputPosition = Vector3.Lerp(mouseInputPosition, resetPosition, Time.deltaTime * ergonomics / 2);
        mouseInputRotation =
            Vector3.Lerp(mouseInputRotation, originalRotation.eulerAngles, Time.deltaTime * ergonomics / 2);
        
        transform.localPosition = Vector3.Lerp(transform.localPosition, newPosition, Time.deltaTime * ergonomics + 10);
        transform.localRotation =
            Quaternion.Lerp(transform.localRotation, newRotation, Time.deltaTime * ergonomics + 10);
    }

    private void CalculateRecoil()
    {
        recoilPosition = transform.localPosition +
                         new Vector3(0, Random.Range(verticalRecoil / 100, verticalRecoil / 100) / 1000, 0);
    ...
    private void MouseInputMovement()
    {
        ...
        mouseInputRotation = new Vector3(mouseX * (ergonomics / Random.Range(0,20000)/10000), mouseY * (ergonomics / Random.Range(0,20000)/10000), 0);
    ...
    private void KeyboardInputMovement()
    {
        ...
        keyboardInputPosition = new Vector3(horizontal / (ergonomics + 30), 0, -vertical / (ergonomics + 30));
